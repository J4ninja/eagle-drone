...
# Scripts.sh
- stream.sh = main GStreamer pipeline. Waits for camera to become available then launch stream.sh Called by systemd on boot and allows only one process to hold camera at a time. 
